...
class Ticket(Base):
    ...
    def create(session, title, conversation):
        ...
        (метод User._get_free_manager) и созданный объект помещает в базу данных.
        В СЛУЧАЕ, ЕСЛИ ВСЕ МЕНЕДЖЕРЫ ОТКАЗАЛИСЬ ОТ ТИКЕТА, ПРИНУДИТЕЛЬНО НАЗНАЧАЕТ НА САМОГО
        СВОБОДНОГО.
        Возвращает 0, если процесс выполнен успешно, и 1, если тикет был назначен принудительно.
        '''
